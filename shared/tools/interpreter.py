...
from super_starter_suite.shared.file_service import FileService
...
class E2BToolOutput(BaseModel):
    is_error: bool
    logs: Any  # From e2b_code_interpreter.models.Logs
...
class E2BCodeInterpreter:
    """
    LOCAL PORT: Fixed version of E2BCodeInterpreter that avoids library hacks.
    Supports redirected output_dir to RAG-ROOT.
    """
    ...
    def _init_interpreter(self, sandbox_files: List[str] = []) -> None:
        ...
        # FIXED: Use Sandbox.create() instead of Sandbox()
        self.interpreter = Sandbox.create(api_key=self.api_key)
        if len(sandbox_files) > 0:
            for file_path in sandbox_files:
                file_name = os.path.basename(file_path)
                # Look for files in uploaded_files_dir if not absolute
                local_file_path = file_path if os.path.isabs(file_path) else os.path.join(self.uploaded_files_dir, file_name)
                
                if os.path.exists(local_file_path):
                    with open(local_file_path, "rb") as f:
                        content = f.read()
                        if self.interpreter and self.interpreter.files:
                            self.interpreter.files.write(file_name, content)
                else:
                    logger.warning(f"File {local_file_path} not found for sandbox upload.")
        ...
        try:
            ...
            for ext, data in zip(formats, results):
                if ext in ["png", "svg", "jpeg", "pdf"]:
                    ...
                    # CUSTOM URL GENERATION: If output_dir is in RAG-ROOT, generate a special URL
                    # This will be handled by our new route in main.py
                    url = document_file.url
                    if "chat_history" in self.output_dir:
                        # Extract relative path to chat_history
                        # /home/.../RAG-ROOT/chat_history/P_financial_report/output -> chat_history/P_financial_report/output
                        # We use /api/files/ prefix
                        parts = self.output_dir.split("chat_history")
                        if len(parts) > 1:
                            relative_path = "chat_history" + parts[1]
                            url = f"/api/files/{relative_path}/{document_file.id}".replace("//", "/")
