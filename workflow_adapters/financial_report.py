...
async def chat_endpoint(request: Request, payload: Dict[str, Any]) -> HTMLResponse:
    """
    Endpoint to handle chat requests for the Financial Report workflow.
    ...
    try:
        ...
        adapter_logger.debug(f"Financial Report workflow request: URL={request.url}  USER={user_id}  SESSION={session_id}  MSG={user_message[:100]}...")
        ...
        if session_id:
            ...
            session = chat_manager.load_session("financial_report", session_id)
            if not session:
                # Create new session if it doesn't exist
                session = chat_manager.create_new_session("financial_report")
        ...
        adapter_logger.error(f"Financial Report workflow error: {str(e)}", exc_info=True)
