...
async def chat_endpoint(request: Request, payload: Dict[str, Any]) -> HTMLResponse:
    ...
    try:
        ...
        adapter_logger.debug(f"Code Generator workflow request: URL={request.url}  USER={user_id}  SESSION={session_id}  MSG={user_message[:100]}...")
        ...
        adapter_logger.error(f"Code Generator workflow error: {str(e)}", exc_info=True)
