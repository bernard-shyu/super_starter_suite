...
async def chat_endpoint(request: Request, payload: Dict[str, Any]) -> HTMLResponse:
    """
    Endpoint to handle chat requests for the ported Deep Research workflow.
    ...
    try:
        ...
        porting_logger.debug(f"Deep Research workflow request: URL={request.url}  USER={user_id}  SESSION={session_id}  MSG={user_message[:100]}...")
        ...
        if session_id:
            ...
            session = chat_manager.load_session("deep_research", session_id)
            if not session:
                # Create new session if it doesn't exist
                session = chat_manager.create_new_session("deep_research")
        ...
        porting_logger.error(f"Deep Research workflow error: {str(e)}", exc_info=True)
