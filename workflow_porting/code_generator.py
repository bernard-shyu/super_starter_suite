...
async def chat_endpoint(request: Request, payload: Dict[str, Any]) -> HTMLResponse:
    ...
    try:
        ...
        if not user_message:
            error_html, status_code = create_error_response("Question is required", "Code Generator", 400)
            return HTMLResponse(content=error_html, status_code=status_code)

        # Get user context from request state
        user_id = request.state.user_id
        user_config = request.state.user_config
        porting_logger.debug(f"Code Generator workflow request: URL={request.url}  USER={user_id}  PAYLOAD={user_message[:100]}...")
