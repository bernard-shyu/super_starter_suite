...
async def chat_endpoint(request: Request, payload: Dict[str, Any]) -> HTMLResponse:
    """
    Endpoint to handle chat requests for the ported Human in the Loop workflow.
    ...
    try:
        ...
        porting_logger.debug(f"Human in the Loop workflow request: URL={request.url}  USER={user_id}  SESSION={session_id}  MSG={user_message[:100]}...")
        ...
        if session_id:
            ...
            session = chat_manager.load_session("human_in_the_loop", session_id)
            if not session:
                # Create new session if it doesn't exist
                session = chat_manager.create_new_session("human_in_the_loop")
        ...
        if hasattr(result, 'response') and result.response:
            ...
            response_content = str(result) if result else "Human in the loop workflow completed successfully"
        ...
        porting_logger.error(f"Human In The Loop workflow error: {str(e)}", exc_info=True)
