...
/**
 * Stage metadata configuration for workflow progress cards
 */
export const createStageMeta = (planningPhase, generationPhase) => ({
  plan: {
    icon: ListChecks,
    badgeText: planningPhase.badgeText,
    gradient: planningPhase.gradient,
    progress: planningPhase.progress,
    iconBg: planningPhase.iconBg,
    badge: planningPhase.badge,
  },
  generate: {
    icon: Wand2,
    badgeText: generationPhase.badgeText,
    gradient: generationPhase.gradient,
    progress: generationPhase.progress,
    iconBg: generationPhase.iconBg,
    badge: generationPhase.badge,
  },
});

/**
 * Predefined stage configurations for common workflows
 */
export const WORKFLOW_STAGE_CONFIGS = {
  ...
  document_generator: createStageMeta(
    {
      badgeText: "Step 1/2: Analyzing",
...
/**
 * Reusable workflow progress card component
 * @param {Object} props
 * @param {Array} props.events - Array of workflow events
 * @param {Object} props.config - Stage configuration object
 * @param {string} props.workflowTitle - Title for the workflow
 */
function WorkflowProgressCard({ events, config, workflowTitle = "Workflow" }) {
