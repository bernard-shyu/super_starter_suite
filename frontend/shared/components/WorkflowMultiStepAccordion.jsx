...
/**
 * Configuration for multi-step workflow phases
 */
...
/**
 * Multi-step workflow accordion component for complex workflows
 * @param {Object} props
 * @param {Array} props.events - Array of workflow events
 * @param {Object} props.config - Workflow configuration object
 */
function WorkflowMultiStepAccordion({ events, config }) {
  ...
  return (
    <div className="mx-auto w-full max-w-4xl space-y-6 p-4">
        ...
        >
          <CardHeader>
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-2">
                <config.phases.retrieve.icon className="h-5 w-5 text-gray-700" />
                <CardTitle>{config.phases.retrieve.title}</CardTitle>
              </div>
              <Badge
                variant="outline"
                className={cn(
            ...
            <CardDescription>
              {config.phases.retrieve.description}
            </CardDescription>
          </CardHeader>
        </Card>
      )}
      ...
      {/* Analyze Panel */}
      {config.phases.analyze && retrieve?.state === "done" && (
        <Card
          className={cn(
            "border-2 transition-all duration-300",
        ...
        >
          <CardHeader>
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-2">
                <config.phases.analyze.icon className="h-5 w-5 text-gray-700" />
                <CardTitle>{config.phases.analyze.title}</CardTitle>
              </div>
              <Badge
                variant="outline"
                className={cn(
            ...
            <CardDescription>
              {config.phases.analyze.description}
            </CardDescription>
          </CardHeader>
        </Card>
      )}
      ...
      {/* Answer Panel */}
      {config.phases.answer && analyze?.state === "done" && answers.length > 0 && (
        <Card>
          <CardHeader>
            <div className="flex items-center space-x-2">
              <config.phases.answer.icon className="h-5 w-5 text-gray-700" />
              <CardTitle>{config.phases.answer.title}</CardTitle>
            </div>
            <CardDescription>
              {config.phases.answer.description}
            </CardDescription>
          </CardHeader>
          <CardContent>
...
export default WorkflowMultiStepAccordion;
