...
class GenerateUICacheManager {
    ...
    async initialize(sessionId = null) {
        try {
            ...
            // Use provided sessionId or get from global scope if available
            const targetSessionId = sessionId || (typeof currentRAGSessionId !== 'undefined' ? currentRAGSessionId : 'default-rag-session');

            // Load cache from backend
            const response = await fetch(`/api/generate/${targetSessionId}/cache/load`, {
                method: 'POST',
    ...
    async save(sessionId = null) {
        ...
        try {
            ...
            const response = await fetch(`/api/generate/${targetSessionId}/cache/save`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' }
            });
...
// Auto-save when page unloads (only if cache manager was properly initialized)
if (typeof document !== 'undefined') {
    window.addEventListener('beforeunload', async () => {
        if (cacheManagerInstance && cacheManagerInstance.isInitialized) {
            ...
            const sessionId = typeof currentRAGSessionId !== 'undefined' ? currentRAGSessionId : 'default-rag-session';
            if (navigator.sendBeacon) {
                const data = JSON.stringify({ action: 'save' });
                navigator.sendBeacon(`/api/generate/${sessionId}/cache/save`, data);
            } else {
                // Fallback to fetch with keepalive
                fetch(`/api/generate/${sessionId}/cache/save`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({}),
                    keepalive: true
