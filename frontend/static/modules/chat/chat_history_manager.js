...
class ChatHistoryManager {
    ...
    bindEvents() {
        ...
        // Action buttons - Chat history UI specific
        document.getElementById('create-new-chat-btn')?.addEventListener('click', () => this.createNewSession());
        ...
        return `
            <div class="session-item" data-session-id="${session.session_id}">
                <div class="session-header">
                    <span class="session-workflow">${this.formatWorkflowName(session.workflow_name)}</span>
    ...
    renderChatMessages() {
        ...
        container.innerHTML = `
            ...
            <div class="resume-chat">
                ...
                <p>Continue chatting with ${this.formatWorkflowName(this.currentSession.workflow_name)}</p>
    ...
    updateSessionTitle() {
        ...
        if (titleElement && this.currentSession) {
            const workflowName = this.formatWorkflowName(this.currentSession.workflow_name);
    ...
    filterSessions() {
        ...
        if (this.searchQuery) {
            filtered = filtered.filter(session => {
                const workflowMatch = session.workflow_name.toLowerCase().includes(this.searchQuery);
    ...
    resumeChat() {
        ...
        console.log(`[ChatHistory] Resuming session ${this.currentSession.session_id} for workflow ${this.currentSession.workflow_name}`);
        ...
        sessionStorage.setItem('resumeSession', JSON.stringify({
            ...
            workflowName: this.currentSession.workflow_name
        }));

        // Use workflow-aware resumption that includes workflow context
        if (window.resumeWorkflowSession) {
            window.resumeWorkflowSession(this.currentSession.session_id, this.currentSession.workflow_name);
        } else {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // NOTE: Moved to modules/chat/ directory for better organization
    console.log('[ChatHistory] Chat History Manager moved to modules/chat/ directory');
});
